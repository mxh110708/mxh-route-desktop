...
export const DEFAULT_PRIORITY_SETTINGS: PrioritySettings = {
  enabled: true, group: "US-West Entry", order: [], failureRounds: 3,
...
export function parsePrioritySettings(value: unknown): PrioritySettings {
  ...
  if (typeof settings.enabled !== "boolean" || typeof settings.group !== "string" || !settings.group.trim()) throw new Error("enabled 必须是布尔值，group 必须是非空分组名称");
